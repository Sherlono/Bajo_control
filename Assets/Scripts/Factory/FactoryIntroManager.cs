...
public class FactoryIntroManager : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (card_index < _info_cards.Length - 1 && _info_cards[card_index].activeSelf == false)
        {
            ...
            {
                _machine.Set_Highlight(false);
                _conveyor.Set_Highlight(true);
                card_index++;
                _info_cards[card_index].SetActive(true);
                _cam_target = new Vector2(cam_targets[card_index].position.x + cam_offset, ui_camera.transform.position.y);
            }
        ...
        if (card_index == 1)
        {
            _scorboy_controller.transform.localPosition = Vector2.Lerp(_scorboy_controller.transform.localPosition, new Vector2(_scorboy_controller.transform.localPosition.x, -23.7f), 0.01f);
        ...
        else _scorboy_controller.transform.localPosition = Vector2.Lerp(_scorboy_controller.transform.localPosition, new Vector2(_scorboy_controller.transform.localPosition.x, -630.0f), 0.01f);
